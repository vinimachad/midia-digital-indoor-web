...
export default function NewsSlideView(props: Slider.News) {
  const { properties } = useNewsSlide(props)

  const Header = () => {
    return (
      <div className={`flex w-screen min-h-[74px] px-8 items-center justify-center ${properties.backgroundColor}`}>
    ...
    return (
      <div className="grid grid-cols-2 items-start gap-x-8 mt-14 px-8">
        ...
        <div className="flex flex-col">
          <h1 className="font-semibold text-6xl text-slate-900">{props.title}</h1>
          ...
          <img className="mt-8 max-w-[300px] relative bottom-0 right-0" src={props.qrcode} />
        </div>
      </div>
    )
  }
