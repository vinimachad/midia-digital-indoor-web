...
export default function useHandlerError() {
  ...
  function handleAPIError(error: AppErrorTypes.ResponseError) {
    const { message, title } = error as AppErrorTypes.APIError
    if (message || title) {
      setAlert({
        show: true,
        title,
  ...
  function handleZodError(error: AppErrorTypes.ZodIssue[]) {
